  ...
  , readOnly_(readOnly)
{
  ...
  pthread_cond_init(&flushed_, nullptr);
...
TFileTransport::~TFileTransport() {
  ...
  if (fd_ > 0) {
    if(-1 == ::close(fd_)) {
      GlobalOutput.perror("TFileTransport: ~TFileTransport() ::close() ",
                          errno);
    } else {
      //successfully closed fd
      fd_ = 0;
...
bool TFileTransport::initBufferAndWriteThread() {
  ...
  if (writerThreadId_ == 0) {
    if (pthread_create(&writerThreadId_,
                       nullptr,
                       startWriterThread,
                       (void *)this) != 0) {
...
void TFileTransport::write(const uint8_t* buf, uint32_t len) {
  if (readOnly_) {
    throw TTransportException("TFileTransport: attempting to write to file "
                              "opened readonly");
  }
...
void TFileTransport::enqueueEvent(const uint8_t* buf, uint32_t eventLen) {
  ...
  toEnqueue->eventBuff_ = new uint8_t[eventLen + 4];
...
bool TFileTransport::swapEventBuffers(struct timespec* deadline) {
  ...
  if (!enqueueBuffer_->isEmpty()) {
    swap = true;
  } else if (closing_ || forceFlush_) {
    // even though there is no data to write, return immediately if the
    // transport is closing or needs to perform an immediate flush
    swap = false;
  } else {
    if (deadline != nullptr) {
    ...
    while(readState_.bufferPtr_ < readState_.bufferLen_) {
      if (readState_.readingSize_) {
        ...
        if (readState_.eventSizeBuffPos_ == 4) {
          ...
          if (*bitwise_cast<uint32_t *>(readState_.eventSizeBuff_) == 0) {
          ...
          readState_.event_->eventSize_ =
            *bitwise_cast<uint32_t *>(readState_.eventSizeBuff_);
...
bool TFileTransport::isEventCorrupted() {
  ...
  if ( (maxEventSize_ > 0) &&  (readState_.event_->eventSize_ > maxEventSize_)) {
    // 1. Event size is larger than user-specified max-event size
...
void TFileTransport::seekToChunk(int32_t chunk) {
  ...
  if (seekToEnd) {
    ...
    // keep on reading until the last event at point of seekChunk call
